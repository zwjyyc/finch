...
## Style of My Code
Other deep learning projects on GitHub usually have several scripts such as ```train.py```, ```model.py```, ```main.py``` and so on. However, personally I'd like to build a model with ```fit()``` and ```predict()``` interfaces (Sklearn API Style), and write separate test files for different data.
## Contents
