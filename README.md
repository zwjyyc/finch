...
## Style of My Code
Other deep learning repos usually have several scripts such as ```train.py```, ```model.py```, ```main.py``` and so on. However, personally I'd like to build a model with ```fit``` and ```predict``` interfaces (like Sklearn), and write separate test files for different dataset.
## Contents
