...
## Style of My Code
Most deep learning models on GitHub usually have several scripts such as ```train.py```, ```model.py```, ```main.py``` and so on. Here, to be concise and minimal, I convert each model to a single class with ```fit()``` and ```predict()``` methods (scikit-learn API style), and then write separate test files for different incoming data. All the code below follow this style.
## Contents
...
#### Recommender System
* Pandas &nbsp; | &nbsp; Collborative Filtering &nbsp; | &nbsp; MovieLens &nbsp; &nbsp; [Functions](https://github.com/zhedongzheng/finch/blob/master/ir-models/python/ncf.py) &nbsp; &nbsp; [Test](https://github.com/zhedongzheng/finch/blob/master/ir-models/python/ncf_movielens_test.py) &nbsp; | &nbsp;
