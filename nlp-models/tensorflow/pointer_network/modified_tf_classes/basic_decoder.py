...
class BasicDecoder(decoder.Decoder):
  ...
  def __init__(self, cell, helper, initial_state, encoder_in, output_layer=None):
  ...
  def step(self, time, inputs, state, name=None):
    ...
    # modification here
    weights = tf.matmul(self._encoder_in, tf.expand_dims(cell_state[-1].h, 2))
    weights = tf.nn.softmax(tf.squeeze(weights, 2))

    pointers = tf.expand_dims(tf.argmax(weights, -1), 1)
    B = tf.range(tf.shape(self._encoder_in)[0])
