...
from rnn_attn_estimator import build_tf_estimator
...
def main():
    (X_train, y_train), (X_test, y_test) = tf.contrib.keras.datasets.imdb.load_data(num_words=args.vocab_size)
    X_train, X_train_lens = zero_pad(X_train)
    X_test, X_test_lens = zero_pad(X_test)
    print("Data Processed")

    tf_estimator = build_tf_estimator()
    ...
    for epoch in range(args.num_epochs):
        ...
        print("Epoch %d | Testing Accuracy: %.3f" % (epoch+1, res['test_acc']))

        tf_estimator = build_tf_estimator(model_dir=tf_estimator.model_dir)
# end function main()
