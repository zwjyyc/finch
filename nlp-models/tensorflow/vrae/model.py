...
class VRAE:
    ...
    def _encoder_to_latent(self):
        ...
        with tf.variable_scope('encoder'):
            ...
            encoded_output, encoded_state = tf.nn.dynamic_rnn(
                ...
                inputs = tf.nn.embedding_lookup(tied_embedding, self.seq),
    ...
    def _latent_to_decoder(self, rnn_encoded):
        with tf.variable_scope('latent'):
            ...
            self.gaussian_noise = tf.random_normal(tf.shape(self.z_logvar))
    ...
    def _parse_decoder_state(self, z, activation=tf.nn.elu):
        if args.rnn_cell == 'lstm':
