...
class Conv1DClassifier:
    def __init__(self, seq_len, vocab_size, embedding_dims, n_out, sess):
    ...
    def build_graph(self):
        ...
        with tf.variable_scope('forward_path'):
            ...
            self.add_conv_layer('conv', filter_shape=[3,self.embedding_dims,250], in_layer=self.embedding_out)
            self.add_maxpool_layer(k=2)
            self.add_fc_layer('fc', [int(self.seq_len/2)*250,50], flatten_input=True)
        with tf.variable_scope('output_layer'):
            self.add_output_layer(in_dim=50)   
    ...
    def add_conv_layer(self, name, filter_shape, in_layer=None):
        if in_layer is None:
            in_layer = self.conv
        self.conv = self.conv1d_wrapper(in_layer, self._W(name+'_w', filter_shape),
