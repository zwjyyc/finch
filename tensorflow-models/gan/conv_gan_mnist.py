...
class ConvGAN:
    ...
    def add_Generator(self):
        # 100 -> (7, 7, 256) -> (14, 14, 128) -> (28, 28, 1)
        # 100 -> (7, 7, 256)
        X = tf.layers.dense(self.G_in, 7 * 7 * 256)
        X = tf.reshape(X, [-1, 7, 7, 256])
        X = tf.nn.relu(tf.contrib.layers.batch_norm(X, is_training=self.train_flag))
        # (7, 7, 256) -> (14, 14, 128)
        Y = tf.layers.conv2d_transpose(X, 128, [5, 5], strides=(2, 2), padding='SAME')
