...
class MLP_GAN:
    ...
    def add_Generator(self):
        ...
        G_hidden = tf.layers.dense(G_in, 128, tf.nn.relu)
    ...
    def add_Discriminator(self):
        ...
        D_hidden = tf.layers.dense(X_in, 128, tf.nn.relu, name='hidden')
        self.X_true_prob = tf.layers.dense(D_hidden, 1, tf.nn.sigmoid, name='out')

        G_out = tf.concat([self.G_out, self.labels], 1)
        D_hidden = tf.layers.dense(G_out, 128, tf.nn.relu, name='hidden', reuse=True)
