...
class RNNClassifier:
                    ...
                    if self.stateful:
                        if local_step == 0:
                            v_loss, v_acc, next_state = self.sess.run([self.loss, self.acc, self.final_state],
                                feed_dict = self.get_val_dict(X_test_batch, y_test_batch, batch_size))
                        else:
                            v_loss, v_acc, next_state = self.sess.run([self.loss, self.acc, self.final_state],
                                feed_dict = self.get_val_dict(X_test_batch, y_test_batch, batch_size,
                                    state=next_state))
    ...
    def predict(self, X_test, batch_size=32):
        batch_pred_list = []
        if self.stateful:
            for n_batch, X_test_batch in enumerate(self.gen_batch(X_test, batch_size)):
                if n_batch == 0:
    ...
    def get_train_op_dict(self, X_batch, y_batch, batch_size, lr, keep_prob_tuple, state=None):
    ...
    def get_val_dict(self, X_batch, y_batch, batch_size, state=None):
        if (self.stateful) and (state is not None):
            return {self.X: X_batch, self.y: y_batch, self.batch_size: batch_size, self.in_keep_prob: 1.0,
                    self.out_keep_prob: 1.0, self.init_state: state}
        else:
            return {self.X: X_batch, self.y: y_batch, self.batch_size: batch_size, self.in_keep_prob: 1.0,
    ...
    def get_pred_dict(self, X_test_batch, batch_size, state=None):
        if (self.stateful) and (state is not None):
            return {self.X: X_test_batch, self.batch_size: batch_size, self.in_keep_prob: 1.0,
                    self.out_keep_prob: 1.0, self.init_state: state}
        else:
            return {self.X: X_test_batch, self.batch_size: batch_size, self.in_keep_prob: 1.0,
                    self.out_keep_prob: 1.0}
    # end method get_pred_dict
